...
module.exports = class YamaServicesMock {
    ...
    getArtistTopAlbums(arId, query){
        if(arId==='bfcc6d75-a6a5-4bc6-8282-47aec8531818' || (query && query.artist==='Cher')) Promise.resolve(albums)
    ...
    putPlaylistMusic(pId, body){
        if(body.artist !== album.artist) return Promise.reject({code:400, message:'Artist is not Cher'})
