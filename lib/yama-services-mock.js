...
module.exports = class YamaServicesMock {
    ...
    postPlaylist(body,cb){
        ...
        return cb(null, playlists[playlists.length-1])
    }
    ...
    putPlaylistMusic(pId, body, cb){
        if(body.artist !== album.artist) return cb({code:400, message:'Artist is not Cher'})
        if(track.name != body.name) return cb({code:404, message:'No track with such name found'})
        let playlist = playlists.find((playlist)=>playlist.id==pId)
        if(playlist!=null){
