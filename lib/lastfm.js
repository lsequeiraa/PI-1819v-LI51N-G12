...
class Lastfm{
    static getArtists(query,cb){
        ...
        if(options.artist === '') return cb({code:400,message:'No artist name specified.'})
    ...
    static getArtistTopAlbums(arID,query,cb){
        let fields = ['artist','limit', 'page']
        let options= checkFields(query,fields)
        if(arID === '' && options.artist === '') return cb({code:400,message:'Please specify an artist name or an mbid of an artist.'})
    ...
    static getAlbum(alid,cb){
        if(alid === '') return cb({code:400,message:'No artist mbid specified.'})
        const url = `?method=album.getinfo&mbid=${alid}`
        getRequest(baseUrl + url,cb)
    }

    static getTrack(mId, cb) {
        if (mId === '') return cb({code:400,message:'No track mbid specified.'});
        const url = `?method=track.getInfo&mbid=${mId}`;
        getRequest(baseUrl + url, cb);
    }
}
