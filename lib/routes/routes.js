...
function postPlaylist(app) {
    ...
    app[METHOD.toLocaleLowerCase()](PATH, isLoggedIn, checkContentType, function (req, resp) {
        ...
        req.on('end',()=> {
            ...
            yama.postPlaylist(req.user.id, body)
                .then((data) => generalThener(resp, data))
                .catch((err) => generalCatcher(resp, err));
        });
    })
...
function getPlaylists(app) {
    ...
    app[METHOD.toLocaleLowerCase()](PATH, isLoggedIn, function (req, resp) {
        debug('Routed to: GET /playlists');

        yama.getPlaylists(req.user.id)
            .then((data) => generalThener(resp, data))
            .catch((err) => generalCatcher(resp, err));
    })
}
